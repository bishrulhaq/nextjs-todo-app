      ...
      <section class="bg-gray-50 dark:bg-gray-900">
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
          <a href="#" class="flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white">
            <img class="w-8 h-8 mr-2" src={toDoLogo.src} alt="logo" />
            ToDo App
          </a>
          <div class="w-full p-6 bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md dark:bg-gray-800 dark:border-gray-700 sm:p-8 mb-2">
            <form onSubmit={addTodo}>
              ...
              <button type="submit" class="w-full mt-5 g-white bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow text-center">
                Add Task
              </button>
            </form>
          </div>
          ...
          {todos.map((todo) => (
            <div key={todo.id} class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md dark:bg-gray-800 dark:border-gray-700 mb-2">
              <div class="flex flex-col md:flex-row items-center justify-center p-4">
                <div class="md:w-3/4">
                  <p onClick={() => markTodo(todo.id)} class={`${todo.done ? 'line-through' : ''}`}>{todo.text}</p>
                </div>
                <div class="md:w-1/4 flex flex-col justify-center items-center">
                {todo.done ? (
                  <button class="text-white font-bold py-2 px-4" disabled>
                    Completed
                  </button>
                  ) : (
